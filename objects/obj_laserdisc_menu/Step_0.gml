...
if sign(vinput)!=0 and menu_cool=0
	{
		...
		if menu_y_selection=clamp(menu_y_selection,0,max(0,counter-1))
			{
...
menu_y_selection=clamp(menu_y_selection,0,max(0,counter-1));
...
scr_HUD_tips_clear();
scr_HUD_set_tips_text(scr_return_text(441), scr_return_text(442));
