...
if (destroyed)
	{
		if destroyedtimer>90+irandom(abs(120-destroyedtimer))
			{
...
_num+=collision_line_list(x,y-1,x+sprite_width,y-1,parent_push,0,1,_list,false);
var i;
for (i=0; i<_num; i+=1)
	{
		var inst=_list[| i];
		//This will remove the instance from the list if its below the surface of the platform
		//Don't read this if statement because it's really terrible
		if (inst.bbox_bottom>=y or inst.yspeed<0 or (object_is_ancestor(inst,parent_possessable) and (inst.interactwithsemisolidsenabled==false or inst.state=PlayerState.defeated)))
			{
				...
				repeat(particleNum)
					{
						...
						part_particles_create_colour(global.partSys,xx,yy,global.platformParticle,c_navy,1);
					}
			}
	}
instancesontop=_num;
		...
		for (i=0; i<_num; i+=1)
			{
				with (_list[| i])
					{
						repeat(ceil(abs(yspd)))
							{
								if (!place_meeting(x,y+sign(yspd),obj_solid))
									{
										y+=sign(yspd);
									}
							}
...
if x!=clamp(x,384-sprite_width,833) and xspeed!=0 and bossstate!=BOSS_STATE.DEFEATED
	{
		instance_destroy();
	}*/
	
if (x!=clamp(x,minx,maxx))
	{
		instance_destroy();
	}
if (y!=clamp(y,miny,maxy))
	{
		instance_destroy();
	}