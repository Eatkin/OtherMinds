...
spr[0]=-1;
description[0]=scr_return_text(67);
...
var linkedInventory=global.inventory;
if (!isPlayer)
	linkedInventory=global.currentinstance.localinventory;
	
//Now we loop through the todo to get all the text
var len=ds_grid_height(linkedInventory);
var textNum=0;

for (var i=0; i<len; i+=1)	{
	if (linkedInventory[# INVENTORY.QUANTITY, i]>0)	{
		text[textNum]		=linkedInventory[# INVENTORY.ITEMNAME, i];
		spr[textNum]		=linkedInventory[# INVENTORY.SPRITE, i];
		description[textNum]=linkedInventory[# INVENTORY.DESCRIPTION,i];
		//Add the quantity to the end of the string if the quantity is bigger than 1
		var quant			=linkedInventory[# INVENTORY.QUANTITY,i];
		if (quant>1)
			text[textNum]+=" x"+string(quant);
		textNum+=1;		
...
for (i=0; i<len; i+=1)	{
	button[i]=instance_create_layer(xx,yy+buttonSpacing*i,layer,obj_inventoryButton);
	...
	button[i].sprite	=spr[i];
	button[i].linkedList=linkedInventory
