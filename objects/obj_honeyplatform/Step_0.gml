...
for (i=0; i<_num; i+=1)
	{
		...
		if (inst.bbox_bottom>=y or inst.yspeed<0 or inst.interactwithsemisolidsenabled==false)
			{
				...
				repeat(particleNum)
					{
						...
						part_particles_create(global.partSys,xx,yy,global.honeyParticle,1);
					}
			}
	}
instancesontop=_num;
...
//Check we're not on camera when disappearing
if (x!=clamp(x,minx,maxx) or y!=clamp(y,miny,maxy))
	if (!scr_on_camera())
		instance_destroy();