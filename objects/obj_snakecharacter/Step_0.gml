...
//Here's the code for creating the key card piece
if (state==PlayerState.dead and timeinstate==0)
	{
		//Check how many snakes aren't dead
		var snakesAlive=0;
		var snakeAliveInst=noone;
		for (var i=0; i<instance_number(obj_snakecharacter); i+=1)
			{
				var snakeInst=instance_find(obj_snakecharacter,i);
				if (snakeInst.state!=PlayerState.dead)
					{
						snakesAlive+=1;
						snakeAliveInst=snakeInst;
					}
			}
		//If there's exactly one snake alive, we saved it's instance number
		if (snakesAlive==1)
			{
				//Select item to give to snake
				var item;
				switch (room)
					{
						case rm_office_vent_room_one:
							item=obj_keycard_D_shard1;
							break;
						case rm_office_vent_room_two:
							item=obj_keycard_D_shard2;
							break;
						case rm_office_vent_room_three:
							item=obj_keycard_D_shard3;
							break;
						default:
							item=noone;
							break;
				...
				if (item!=noone)
					{
						with (snakeAliveInst)
							{
								//Add all references
								scr_inventory_add_ref(obj_keycard_D_shard1,scr_return_text(552),0,spr_key_card_D_piece1,scr_return_text(554),localinventory);
								scr_inventory_add_ref(obj_keycard_D_shard2,scr_return_text(552),0,spr_key_card_D_piece2,scr_return_text(554),localinventory);
								scr_inventory_add_ref(obj_keycard_D_shard3,scr_return_text(552),0,spr_key_card_D_piece3,scr_return_text(554),localinventory);
								scr_inventory_add(item,localinventory);
							}
					}
			}
		
		var allsnakesdead=true;
		with (obj_snakecharacter)
			{
				allsnakesdead*=(state==PlayerState.dead);
			}
		if (allsnakesdead==true)
			{
				var event;
				var item;
				//select event and item based on room
				switch (room)
					{
						case rm_office_vent_room_one:
							{
								event=OFFICEEVENTS.VENTROOM1_SNAKES;
								item=obj_keycard_D_shard1;
								break;
							}
						case rm_office_vent_room_two:
							{
								event=OFFICEEVENTS.VENTROOM2_SNAKES;
								item=obj_keycard_D_shard2;
								break;
							}
						case rm_office_vent_room_three:
							{
								event=OFFICEEVENTS.VENTROOM3_SNAKES;
								item=obj_keycard_D_shard3;
								break;
							}
						default:
				...
				if (item!=noone)
					{
						global.officeevents=scr_event_set_complete(global.officeevents,event);
						instance_create_layer(xsafe,ysafe,layer,item);
						var alleventscomplete=true;
						var event1=scr_event_check_complete(global.officeevents,OFFICEEVENTS.VENTROOM1_SNAKES);
						var event2=scr_event_check_complete(global.officeevents,OFFICEEVENTS.VENTROOM2_SNAKES);
						var event3=scr_event_check_complete(global.officeevents,OFFICEEVENTS.VENTROOM3_SNAKES);
						alleventscomplete=event1*event2*event3;
						if (alleventscomplete==true)
							{
